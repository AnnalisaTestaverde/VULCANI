// ===== CONFIGURAZIONE GRAFICO D'INSIEME COMPLETO =====
const CONFIG = {
    colors: {
        background: '#FF2B00', // Sfondo arancione per la pagina learn more
        text: '#000000ff',
        accent: '#FFFFFF', // Bianco per titolo, bordi bottoni
        circle: '#111010b3',
        infoBox: '#ffffffff',
...
function initializeData() {
    ...
    state.showMethodologyButton = false; // Inizialmente nascosto
}
...
// 8 - Disegno cerchi di impatto (IDENTICO all'originale)
...
// 10 - Aggiornamento animazioni timeline
...
// 11 - Aggiornamento animazioni punti
...
function updateCursor() {
    ...
    if ((state.backButtonArea &&
        ...
        mouseY > state.backButtonArea.y &&
        mouseY < state.backButtonArea.y + state.backButtonArea.height) ||
        (state.showMethodologyButton && state.methodologyButtonArea &&
        mouseX > state.methodologyButtonArea.x &&
        mouseX < state.methodologyButtonArea.x + state.methodologyButtonArea.width &&
        mouseY > state.methodologyButtonArea.y &&
        mouseY < state.methodologyButtonArea.y + state.methodologyButtonArea.height)) {
...
function legendSketch(sketch) {
    sketch.setup = function() {
        ...
        const canvas = sketch.createCanvas(container.clientWidth, container.clientHeight);
    ...
    sketch.draw = function() {
        ...
        sketch.fill(255);
        sketch.text('Eruption order: from oldest to most recent within each continent. ', startX + 45, y3 + 12);
    };
}
